...
class CgsmsSecurityTwoFactor
{
    ...
    public static function getRoles()
    {
        ...
        $current_roles = get_option('cgsms_security_required_roles') ? : [];
    ...
    {
        $code = get_option('cgsms_security_bypass_code');
        if (!$code) {
            $code = strtolower(wp_generate_password(32, false, false));
            update_option('cgsms_security_bypass_code', $code, false);
        }
    ...
    public static function getLoginDataByTempToken($temp_token)
    {
        ...
        if (!count($parts) == 2) return new WP_Error('INVALID_TOKEN', __('Error: The format of the temporary token is invalid.', 'cheapglobalsms'));

        $token_data = get_transient('cgsms_2f_' . $temp_token);
        if (!$token_data) return new WP_Error('EXPIRED', __('Error: Your token seems to have expired. Please start over.', 'cheapglobalsms'));
    ...
    public static function checkIfThrottled($user_ID, $action, $max_attempts, $expires_after_seconds)
    {
        ...
        if (count($throttle) >= $max_attempts) {
            ...
            $error_msg = apply_filters('cgsms_throttle_error', __('Your account is temporarily locked, due to too many verification attempts in a short time span.<br /><br />Please wait at least :time_left before trying again.', 'cheapglobalsms'), $user_ID, $action, $max_attempts, $expires_after_seconds);
    ...
    public static function createClientCookie($user_ID)
    {
        ...
        $cookie_lifetime = (int)get_option('cgsms_security_cookie_lifetime', 0);
    ...
    public static function enqueueCssJs(){
        add_action('login_enqueue_scripts', function () {
			...
			<script type="text/javascript">
				var CGSMS_PLUGINDIR = <?= json_encode(_cgsms_url()); ?>;
				var CGSMS_I18N_DEFAULT_ERROR = <?= json_encode(__('Sorry, but there are errors in your input. Please attend to the highlighted fields below.', 'cheapglobalsms'));?>;
				var CGSMS_ADMINURL = <?= json_encode(admin_url()); ?>;
			</script>
			<?php
		    wp_enqueue_script('jquery');			
			wp_register_script('cgsms-widgets', _cgsms_url() . 'js/widgets.js', ['jquery','underscore'], CHEAPGLOBALSMS_VERSION, true);
            wp_enqueue_script('cgsms-widgets', _cgsms_url() . '/js/widgets.js');
            wp_enqueue_script('cgsms-wp-login', _cgsms_url() . '/js/wp-login.js');
            wp_enqueue_style('cgsms-wp-login', _cgsms_url() . '/css/wp-login.css');

            $i18n = [
                'ajax_tech_error' => __('An unknown technical error occured while processing the request. Please try again or contact your administrator.', 'cheapglobalsms')
            ];
            ?>
            <script>
    ...
    public static function enableBypassMode()
    {
        ...
        if (!isset($_GET['c']) || $_GET['c'] != $bypass_code) {
            wp_die('<h1>' . __('Bypass code is invalid!', 'cheapglobalsms') . '</h1><p>' . __('Your two-factor bypass code in the URL, is invalid. Two-factor security is still enabled.', 'cheapglobalsms') . '</p>');
        }
        ...
        ob_start();
        ?>
        <strong><?php _e('Two-factor bypass mode active', 'cheapglobalsms'); ?></strong><br><br/>
        <?php _e('Issues related to two-factor, should be temporarily resolved if you log in now.', 'cheapglobalsms'); ?>
        <?php
        global $error;
        $error = ob_get_clean();
    }
...
class CgsmsSecurityTwoFactorAddMobile
{
    ...
    public static function handleAddMobile(\WP_user $user)
    {
        ...
        include _cgsms_dir() . '/tpl/wp-login-add-phone.php';
    }
    ...
    public static function sendInitialSms()
    {
        ...
        $message = strtr(__("Verification code: :code\nKind regards, :sender\n:home_url", 'cheapglobalsms'), [':code' => $login_info['code'], ':sender' => $sender, ':home_url' => $home_url]);
        $status = cgsms_send_sms($message, $msisdn);
        ...
        if (is_wp_error($status)) {
            if ($status->get_error_code() == 'CGSMS_FAIL') {
                ...
                CgsmsSecurityTwoFactor::jsonFail(new WP_Error('cheapglobalsms', strtr(__('The SMS could not be sent, due to the SMS-service rejecting to send the message.<br><br>Technical reason:<br />- :reason', 'cheapglobalsms'), [':reason' => $reason])));
            } else {
                CgsmsSecurityTwoFactor::jsonFail(new WP_Error('cheapglobalsms', __('The SMS could not be sent due to a technical error/misconfiguration of the CheapGlobalSMS-plugin.<br /><br />You could try again, but probably this can only be resolved by your administrator.', 'cheapglobalsms')));
            }
        }
    ...
    {
        ob_start();
        include _cgsms_dir() . '/tpl/wp-login-confirm-phone.php';
        return ob_get_clean();
    }
    ...
    public static function confirmSms()
    {
        ...
        add_filter('cgsms_throttle_error', function ($e) {
            return __('You have tried to enter the confirmation token too many times. Please start over to try again.', 'cheapglobalsms');
        });
        ...
        if ($user_code != $correct_code) {
            CgsmsSecurityTwoFactor::jsonFail(new WP_Error('BAD_CODE', __('The code you have entered, is invalid. Please double check the SMS we sent you and try again.', 'cheapglobalsms')));
        }
    ...
    {
        ob_start();
        include _cgsms_dir() . '/tpl/wp-login-confirmed-phone.php';
        return ob_get_clean();
    }
}
...
class CgsmsSecurityTwoFactorHasMobile
{
    ...
    public static function handleLogin(\WP_User $user)
    {
        ...
        $sender = get_bloginfo();
        $home_url = url_shorten(get_home_url());
        $message = strtr(__("Verification code: :code\nKind regards, :sender\n:home_url", 'cheapglobalsms'), [':code' => $code, ':sender' => $sender, ':home_url' => $home_url]);
        ...
        if (is_wp_error($status)) {
            $main_reason = '<h1>' . __('Problem in the two-factor security module', 'cheapglobalsms') . '</h1>';
            ...
            if ($status->get_error_code() == 'CGSMS_FAIL') {
                ...
                wp_die(new WP_Error('cheapglobalsms', $main_reason . '<p>' . strtr(__('The SMS could not be sent, due to the SMS-service rejecting to send the message.<br><br>Technical reason:<br />- :reason', 'cheapglobalsms'), [':reason' => $reason]) . '</p>'));
            } else {
                wp_die(new WP_Error('cheapglobalsms', $main_reason . '<p>' . __('The SMS could not be sent due to a technical error/misconfiguration of the CheapGlobalSMS-plugin.<br /><br />You could try again, but probably this can only be resolved by your administrator.', 'cheapglobalsms') . '</p>'));
            }
        }
    ...
    private static function getHtmlLoginConfirmPhone($mcc, $mno)
    {
        ...
        // output!
        login_header(__('Two-factor security check', 'cheapglobalsms'));
...
class CgsmsSecurityTwoFactorUserProfile
{
    public static function addContactMethod($methods, $user = null)
    {
        ...
        $methods['cgsms_msisdn'] = __('Mobile number', 'cheapglobalsms');
    ...
    public static function enqueueJsCss()
    {
        ...
        wp_register_script('wpadmin-profile-two_factor', _cgsms_url() . '/js/wpadmin-profile-two_factor.js', ['jquery']);

        $nonce = wp_create_nonce('cgsms_profile_change_phone');
        $user = wp_get_current_user();

        $i18n = [
            'twofac_section_h1' => __('Two-factor security', 'cheapglobalsms'),
            'twofac_section_intro' => __('For security reasons, it is mandatory to associate your account with a cellphone number. This greatly enhances the integrity of your account and this site in general.', 'cheapglobalsms'),
            'twofac_add_number' => __('Add mobile number', 'cheapglobalsms'),
            'twofac_update_number' => __('Change mobile number', 'cheapglobalsms'),
    ...
    public static function gotoChangeMobile()
    {
        ...
        if (!wp_verify_nonce($_GET['_nonce'], 'cgsms_profile_change_phone')) {
            wp_die(__('You have followed a link containing an expired/invalid nonce. Please go back and redirect the page - then it should work.', 'cheapglobalsms'));
        }
